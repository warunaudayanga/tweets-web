...
export class AuthService {
    ...
    login(dto: LoginDto): Observable<AuthResponse> {
        return this.http.post<AuthResponse>(`${Endpoint.AUTH}/${AuthEndpoint.LOGIN}`, dto).pipe(
            take(1),
            map(res => ({
                ...res,
    ...
    register(dto: CreateUserDto): Observable<User> {
        return this.http.post<User>(`${Endpoint.AUTH}/${AuthEndpoint.REGISTER}`, dto).pipe(take(1));
    }
